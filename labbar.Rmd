...
fittan <-df[[1]][c(1:9,12,11)]
andra <-fittan[,c(10:11)]
fittan <-fittan[,1:9]
fittan <-fittan[rev(positioner)]
fittan <-cbind(fittan,andra)
fittan <-kable(fittan,format="latex")
fittan
...
pknull <-kable(pvals,format="latex")
pknull
```
...
pfithora <-kable(pfit,format="latex")
binom <-function(n,k,p){
...
kukar <-kable(meansen,format = "latex")
kukar
```
...
knlla <-kable(medel,format="latex")
knlla
```
...
pfit <-pfit[c(7,8,4,9,1,2,3,5,6),c(2,1)]
pfit$Pval <-round(pfit$Pval,digits=3)
pfitta <-kable(pfit,format = "latex")
pfitta
